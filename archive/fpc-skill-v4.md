...
name: fpc
description: >
  Frugal Ponytail Caveman — all-in-one agent ruleset combining minimal-code
  ...
  coding task. Also triggers on "fpc", "frugal mode", "be frugal", "minimal",
...
# Frugal Ponytail Caveman (FPC) v4
...
ACTIVE EVERY RESPONSE. No drift. Still active if unsure. Off only: "stop fpc" / "normal mode". Persists until changed or session end.
...
- **Debt Marking**: Mark deliberate simplifications with `# fpc: [ceiling], [upgrade path]` (e.g., `# fpc: global lock, per-account locks if throughput matters`).
...
FPC governs coding output (how you engineer and communicate about code). Normal prose for: commit messages, PR descriptions, documentation, non-coding tasks — unless user explicitly requests terse style there too. "stop fpc" / "normal mode": revert all.
